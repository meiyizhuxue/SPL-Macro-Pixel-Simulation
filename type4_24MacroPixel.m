...
% 第4种类型：2*4宏像素
N_subpixel = 8;
SNR_24MacroPixel = zeros(4, N_subpixel, 4);
Hst_24MacroPixel = cell(4, N_subpixel, 4);
for n = 1:length(ambientLightIn)
    ...
    for th = 1:N_subpixel
        for W = 1:4
            ...
            Hst_24MacroPixel{n, th, W} = countsHistogram;
            % 计算SNR
            ...
            SNR_24MacroPixel(n, th, W) = SNR_values;
        end
    end
end
% 绘图
...
for n = 1:length(ambientLightIn)
    subplot(2, 2, n);
    subplot(2,2,n);
    temp = squeeze(SNR_24MacroPixel(n, :, :));
    % 获取各个背景光强度下的最大的信噪比及SNR对应的阈值和重合检测时间
    [maxVal, linearIdx] = max(temp(:));
    [th, W] = ind2sub(size(temp), linearIdx);
    % 绘制条形图并保存句柄
    bar(activeBins, Hst_24MacroPixel{n, th, W}, 'FaceColor', barColor, 'EdgeColor', 'none', ...
        'BarWidth', 2, 'FaceAlpha', 0.8);
    hold on;
    
    % 创建红色虚线框标记目标区域
    yMax = max(Hst_24MacroPixel{n, th, W});

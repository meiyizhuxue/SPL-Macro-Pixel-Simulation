...
% 第三种情况：2*2宏像素
N_subpixel = 4;
figure('Name', '2*2-MacroPixel');
SNR_22MacroPixel = zeros(4, N_subpixel, 4);
Hst_22MacroPixel = cell(4, N_subpixel, 4);
for n = 1:length(ambientLightIn)
    ...
    for th = 1:N_subpixel
        for W = 1:4
            ...
            Hst_22MacroPixel{n, th, W} = countsHistogram;
            % 计算SNR
            ...
            SNR_22MacroPixel(n, th, W) = SNR_values;
        end
    end
end
% 绘图
% for n = 1:length(ambientLightIn)
%     subplot(2,2,n);
%     temp = squeeze(SNR_22MacroPixel(n, :, :));
%     % 获取各个背景光强度下的最大的信噪比及SNR对应的阈值和重合检测时间
%     [maxVal, linearIdx] = max(temp(:)); 
%     [th, W] = ind2sub(size(temp), linearIdx);
%     bar(activeBins, Hst_22MacroPixel{n, th, W}, 'b', 'BarWidth', 4);
%     hold on;
%     rectangle('Position', [targetBin-100, 0, 200, max(Hst_22MacroPixel{n, th, W})], ...
...
for n = 1:length(ambientLightIn)
    ...
    [th, W] = ind2sub(size(temp), linearIdx);
    % 绘制条形图并保存句柄
    bar(activeBins, Hst_22MacroPixel{n, th, W}, 'FaceColor', barColor, 'EdgeColor', 'none', ...
        'BarWidth', 2, 'FaceAlpha', 0.8);
    hold on;
    
    % 创建红色虚线框标记目标区域
    yMax = max(Hst_22MacroPixel{n, th, W});
    ...
    % 添加标题（学术格式）
   title(sprintf('(%c) Ambient Light: %.1f klux;th: %.0f;W: %.0f| SNR: %.2f', ...
                  char('a' + n - 1), ambientLightIn(n)/1e3, th, W, maxVal), ...

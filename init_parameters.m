function [para, physConst, env] = init_parameters(N_subpixel, ambientLightIn)
    % 发射端参数
    para.tx.energy        = 100 * 1e-8;    % 脉冲激光能量 [J]
    para.tx.pulseWidth    = 4 * 1e-9;      % 脉冲宽度 [s]
    para.tx.divergence    = 20 * 1e-3;     % 激光发散角 [rad]
    para.tx.wavelength    = 532 * 1e-9;    % 波长 [m]
    ...
    para.rx.N_subpixel    = N_subpixel;    % 宏像素拥有的SPAD数
